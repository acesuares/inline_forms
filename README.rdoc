...
That installs the +inline_forms+ engine at the same version as the installer (required — an older +inline_forms_installer+ with a newer +inline_forms+ leaves +validation_hints ~> 6.3+ in the generated +Gemfile+ while the engine needs +validation_hints >= 7.13+). Keep both gems on the same release, e.g. +gem install inline_forms_installer -v 7.13.8+. To install both gems explicitly (e.g. when adding inline_forms to an existing Rails app without the CLI):
